...
3. **Environment Variables:**
   ...
   - Add the following key-value pairs:
     ...
     | `JWT_SECRET` | A unique cryptographically random secret of at least 32 characters | Generate with a password manager; never commit it |
     
   - Ensure the environments checkbox for **Production**, **Preview**, and **Development** are checked.
   - Click **Save**.
   - Never reuse the example value from documentation. Production startup requires `JWT_SECRET` to be configured.
4. **Redeploy Project:**
