...
import {
  ...
  etaComparison,
  weatherRiskScore
} from '../services/trafficCalculations';
...
const LiveTraffic = () => {
  ...
  const [selectedDispatchUnit, setSelectedDispatchUnit] = useState('Dhaka City Traffic Police Unit A');
  ...
  useEffect(() => {
    ...
    const loadTraffic = async () => {
      try {
        ...
        if (alive) {
          const enriched = enrichTraffic(data, featureModules);
          setTraffic(enriched);
  ...
  const loadOperationalData = useCallback(async () => {
    ...
    const [incidentResult, operationResult] = await Promise.all([
      api.get('/incidents?limit=10'),
      api.get('/operations?limit=20')
    ]);
    setOperationalIncidents(incidentResult.data.items || []);
    setOperations(operationResult.data.items || []);
  }, [canManage]);

  useEffect(() => {
    loadOperationalData().catch(() => setWorkspaceMessage('Unable to load protected operations data.'));
  }, [loadOperationalData]);

  const runWorkspaceAction = async (action, successMessage) => {
    if (!canManage) {
      setWorkspaceMessage('Authority or Admin access is required for this action.');
      return;
    }
    ...
    try {
      ...
      logAudit(successMessage);
      await loadOperationalData();
    } catch (error) {
      setWorkspaceMessage(error.response?.data?.message || 'The operation could not be completed.');
    } finally {
        ...
        return (
          <>
            ...
            <div className="sim-result-box">
              ...
              <p>Estimated physical queue length: <strong>{estimateQueue(simQueueCars).meters} meters</strong></p>
              <p>Estimated signal clearing window: <strong>{estimateQueue(simQueueCars).clearanceSeconds} seconds</strong></p>
            </div>
          </>
        );
        ...
        return (
          <>
            ...
            <div className="sim-result-box">
              ...
              <p style={{ marginTop: '8px' }}>Optimized Green Phase: <strong>{adaptiveSignalPlan(simCongestion).greenSeconds}s</strong></p>
              <p>Optimized Clearance Window: <strong>{adaptiveSignalPlan(simCongestion).clearanceSeconds}s</strong></p>
              <p>Estimated capacity improvement: <strong style={{ color: 'var(--primary)' }}>+{adaptiveSignalPlan(simCongestion).improvementPercent}% flow</strong></p>
            </div>
          </>
        );
        ...
        return (
          <>
            <div className="status-list">
              {!canManage && <p className="panel-subtitle">Sign in with an Authority or Admin account to review reports.</p>}
              {canManage && operationalIncidents.filter((incident) => !['Resolved', 'Rejected'].includes(incident.status)).slice(0, 5).map((incident) => (
                <div className="status-item" key={incident._id} style={{ flexDirection: 'column', alignItems: 'stretch', gap: '10px' }}>
                  <div>
                    <span className={`badge ${incident.severity === 'Critical' ? 'danger' : 'warning'}`}>{incident.status}</span>
                    <strong style={{ marginTop: '6px' }}>{incident.title}</strong>
                    <p style={{ fontSize: '0.85rem', color: 'var(--muted)' }}>{incident.locationName}</p>
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button
                      ...
                      onClick={() => runWorkspaceAction(async () => {
                        await api.put(`/incidents/${incident._id}`, { status: 'Investigating' });
                        await api.post('/operations', { category: 'Incident', action: `Verified incident: ${incident.title}`, status: 'In Progress', details: { incidentId: incident._id } });
                      }, `Verified ${incident.title}.`)}
                    ...
                    <button
                      ...
                      style={{ cursor: 'pointer' }}
                      onClick={() => runWorkspaceAction(async () => {
                        await api.put(`/incidents/${incident._id}`, { status: 'Rejected' });
                        await api.post('/operations', { category: 'Incident', action: `Rejected incident: ${incident.title}`, status: 'Completed', details: { incidentId: incident._id } });
                      }, `Rejected ${incident.title}.`)}
                    >Reject</button>
                  </div>
                </div>
              ))}
              {canManage && !operationalIncidents.filter((incident) => !['Resolved', 'Rejected'].includes(incident.status)).length && (
                <p className="panel-subtitle">No reports are awaiting review.</p>
              )}
            </div>
          </>
        );
      ...
      case 11: // Weather impact scoring
        return (
          <>
            ...
            <div className="sim-result-box">
              <p>Calculated Road Risk Index: <strong style={{ color: simWeatherRain > 50 ? 'var(--danger)' : 'var(--primary)' }}>{weatherRiskScore(simWeatherRain)}/100</strong></p>
      ...
      case 26: // ETA comparison
        return (
          <>
            ...
            <div className="sim-result-box">
              <p>🚘 Private Car ETA: <strong>{etaComparison(simETAComparisonDistance).car} mins</strong></p>
              <p>🚊 MRT Feeder ETA: <strong>{etaComparison(simETAComparisonDistance).metro} mins</strong></p>
              <p>🚍 Public Bus ETA: <strong>{etaComparison(simETAComparisonDistance).bus} mins</strong></p>
            </div>
          </>
        );
          ...
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <button
                type="button"
                className="button"
                disabled={workspaceBusy || !canManage}
                onClick={() => runWorkspaceAction(
                  () => api.post('/operations', {
                    category: 'Dispatch',
                    action: `Dispatched ${selectedDispatchUnit} to Paltan Crossing`,
                    status: 'In Progress',
                    details: { unit: selectedDispatchUnit, location: 'Paltan Crossing' }
                  }),
                  `${selectedDispatchUnit} dispatched to Paltan Crossing.`
                )}
              >
          ...
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <button
                type="button"
                className="button"
                disabled={workspaceBusy || !canManage || !simAlertMsg.trim()}
                onClick={() => runWorkspaceAction(async () => {
                  await api.post('/alerts', {
                    title: 'Traffic Operations Advisory',
                    message: simAlertMsg.trim(),
                    area: 'Dhaka Metro Area',
                    severity: 'Warning',
                    active: true
                  });
                  await api.post('/operations', {
                    category: 'Alert',
                    action: `Broadcast commuter warning: ${simAlertMsg.trim()}`,
                    status: 'Completed'
                  });
                }, 'Public alert persisted and broadcast successfully.')}
              >
        ...
        return (
          <>
            <div className="status-list" style={{ maxHeight: '200px', overflowY: 'auto' }}>
              {(canManage ? operations : simAudits).map((audit, idx) => (
                <div
                  key={idx}
                  style={{
                    padding: '10px 14px',
                ...
                >
                  {typeof audit === 'string' ? audit : `[${new Date(audit.createdAt).toLocaleTimeString()}] ${audit.action} (${audit.status})`}
                </div>
              ))}
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: '14px' }}>
              {!canManage && <button type="button" className="button secondary" onClick={() => setSimAudits([])}>Clear Session Logs</button>}
              {canManage && <span className="badge success">Persisted audit records</span>}
            </div>
          </>
        );
          ...
          <div className="workspace-drawer" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="workspace-body">
              <div className="workspace-section">
                ...
                {workspaceMessage && <div className="message" style={{ marginBottom: '14px' }}>{workspaceMessage}</div>}
