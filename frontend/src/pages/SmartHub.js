...
const TRAFFIC_TILE_URL = 'https://api.tomtom.com/traffic/map/4/tile/flow/relative0/{z}/{x}/{y}.png?key=agB2MDvNiThljn4OD0KkLTd9a5TJnhKi';
...
const allTools = [
  // --- USER (Sayed) : The 8 Best / Most Complex Features ---
  ...
  // --- TEAM MEMBER 1 (Mixed Complexity) ---
  ...
  // --- TEAM MEMBER 2 (Mixed Complexity) ---
  ...
  // --- TEAM MEMBER 3 (Mixed Complexity) ---
    ...
    <div className="tools-page">
      <div className="tools-page-header">
        <div>
          ...
          <p>{allTools.length} real tools to help you navigate, plan, and stay safe in Dhaka.</p>
        </div>
        {activeTool && (
    ...
    <div className="pin-route-planner">
      ...
      <div className={`tool-map-wrap pin-route-map ${pickMode ? 'picking' : ''}`}>
        ...
        <MapContainer center={dhakaCenter} zoom={12} scrollWheelZoom style={{ height: 500, width: '100%', borderRadius: 12 }}>
          ...
          {/* TomTom real-time traffic flow — green/orange/red/dark-red */}
          <TileLayer attribution='Traffic &copy; <a href="https://www.tomtom.com">TomTom</a>' url={TRAFFIC_TILE_URL} opacity={0.7} maxZoom={22} />
          ...
          {route && <Polyline positions={route.geometry} pathOptions={{ color: 'var(--primary)', weight: 7, opacity: 1 }} />}
    ...
    <div className="metro-live-guide">
      ...
      <div className="tool-map-wrap metro-live-map">
        ...
        <MapContainer center={[23.79, 90.387]} zoom={12} scrollWheelZoom style={{ height: 520, width: '100%', borderRadius: 12 }}>
          ...
          {kamalapur && <Polyline positions={[motijheel.coords, kamalapur.coords]} pathOptions={{ color: 'var(--muted)', weight: 4, opacity: 0.7, dashArray: '7 8' }} />}
...
const AirQuality = () => {
  ...
  const [locName, setLocName] = useState('Dhaka Center');
  ...
  const fetchAqi = async (lat, lng) => {
    ...
    try {
      ...
      const locRes = await fetch(`https://dataservice.accuweather.com/locations/v1/cities/geoposition/search?apikey=${ACCUWEATHER_API_KEY}&q=${lat},${lng}`);
      const locData = await locRes.json();
      
      if (locData && locData.Key) {
        const weatherRes = await fetch(`https://dataservice.accuweather.com/forecasts/v1/daily/1day/${locData.Key}?apikey=${ACCUWEATHER_API_KEY}&details=true`);
        const weatherData = await weatherRes.json();
        
        if (weatherData && weatherData.DailyForecasts && weatherData.DailyForecasts.length > 0) {
          setData(weatherData.DailyForecasts[0].AirAndPollen);
        }
      }
    } catch (e) {
      console.error(e);
      setLocName('API Error');
      setData(null);
    }
    setLoading(false);
  };
  ...
  useEffect(() => {
    fetchAqi(position.lat, position.lng);
  }, []); // eslint-disable-line
  ...
  const getAQColor = (cat) => {
    const l = (cat || '').toLowerCase();
    if (l === 'good' || l === 'low' || l === 'beneficial') return 'var(--success)';
    if (l === 'moderate' || l === 'neutral') return 'var(--warning)';
    if (l === 'unhealthy' || l === 'high' || l === 'very high' || l === 'extreme') return 'var(--danger)';
    return 'var(--primary)';
  };
  ...
  return (
    <div className="air-quality-tool">
      <div className="tool-intro">
        ...
        <p>Live AccuWeather Air Quality, UV, and Pollen indices for your exact area.</p>
      </div>
      
      <div style={{ position: 'relative', height: '220px', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--line)', marginBottom: '20px' }}>
        ...
        <div style={{ position: 'absolute', top: 10, left: '50%', transform: 'translateX(-50%)', background: 'rgba(13,17,23,0.9)', color: '#fff', padding: '6px 12px', borderRadius: '20px', fontSize: '12px', zIndex: 1000, pointerEvents: 'none', border: '1px solid var(--line)' }}>
          📍 Tap map to check area
        </div>
      </div>

      <div style={{ background: 'var(--surface)', padding: '24px', borderRadius: '12px', border: '1px solid var(--line)' }}>
        <h4 style={{ margin: '0 0 20px 0', fontSize: '18px', textAlign: 'center', color: '#f8fafc' }}>{locName}</h4>
        
        {loading ? (
          <p style={{ textAlign: 'center', color: 'var(--muted)' }}>Loading AccuWeather Indices...</p>
        ) : data ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px' }}>
            {data.map(item => (
              <div key={item.Name} style={{ background: 'var(--bg)', padding: '16px', borderRadius: '8px', border: '1px solid var(--line)', textAlign: 'center' }}>
                <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '8px' }}>{item.Name === 'AirQuality' ? 'Air Quality' : item.Name}</div>
                <div style={{ fontSize: '24px', fontWeight: 'bold', color: getAQColor(item.Category) }}>
                  {item.Category}
                </div>
                {item.Value > 0 && <div style={{ fontSize: '12px', color: '#cbd5e1', marginTop: '4px' }}>Index: {item.Value}</div>}
              </div>
            ))}
          </div>
        ) : (
          <p style={{ textAlign: 'center', color: 'var(--danger)' }}>No data available. Check API Key quota.</p>
        )}
        
        <div style={{ textAlign: 'center' }}>
          <button className="button secondary" onClick={() => fetchAqi(position.lat, position.lng)} disabled={loading} style={{ fontSize: '13px', marginTop: '20px' }}>
            🔄 Refresh
          </button>
          <p style={{ margin: '12px 0 0', fontSize: '11px', color: 'var(--muted)' }}>
            Source: <a href="https://www.accuweather.com" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary)' }}>AccuWeather</a> API
          </p>
        </div>
      </div>
    </div>
...
const Weather = () => {
  ...
  const fetchWeather = async (lat, lng) => {
    ...
    try {
      ...
      if (locData && locData.Key) {
        const weatherRes = await fetch(`https://dataservice.accuweather.com/currentconditions/v1/${locData.Key}?apikey=${ACCUWEATHER_API_KEY}&details=true`);
        const weatherData = await weatherRes.json();
        if (weatherData && weatherData.length > 0) setData(weatherData[0]);
      }
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div className="weather-tool">
      <div className="tool-intro">
        ...
        <p>Live AccuWeather updates for Dhaka areas.</p>
      </div>
      ...
      <div style={{ textAlign: 'center', padding: '40px 20px', background: 'rgba(47, 191, 113, 0.12)', borderRadius: '12px', color: 'var(--success)' }}>
        <h4 style={{ margin: '0 0 16px 0', fontSize: '18px' }}>{locName}</h4>
        {loading ? (
          <p>Loading AccuWeather data...</p>
        ) : data ? (
          <>
            <div style={{ fontSize: '48px', fontWeight: 'bold', color: 'var(--success)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
              {Math.round(data.Temperature.Metric.Value)}°C
            </div>
            <p style={{ margin: '8px 0 24px 0', color: 'var(--success)', fontSize: '16px' }}>
              {data.WeatherText} • Wind: {Math.round(data.Wind.Speed.Metric.Value)} km/h • Humidity: {data.RelativeHumidity}%
            </p>
            <p style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '8px' }}>
              Source: <a href={data.Link} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary)' }}>AccuWeather</a>
            </p>
          </>
        ) : (
          <p>No data available. Check API Key quota.</p>
        )}
      </div>
    </div>
  );
...
const OfflineSms = () => {
  ...
  return (
    <div className="offline-sms-tool">
      ...
      <form onSubmit={handleSubscribe} className="rp-form" style={{ background: 'var(--surface)', padding: '20px', borderRadius: '12px', border: '1px solid var(--line)' }}>
        <label>
...
const RainWarning = () => {
  ...
  const fetchSuggestions = async (query) => {
    ...
    try {
      const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query + ', Dhaka, Bangladesh')}&limit=5`);
      const data = await res.json();
      setSuggestions(data);
  ...
  const checkRainByCoords = async (lat, lon, locName) => {
    setChecking(true);
    setResult(null);
    try {
      const locRes = await fetch(`https://dataservice.accuweather.com/locations/v1/cities/geoposition/search?apikey=${ACCUWEATHER_API_KEY}&q=${lat},${lon}`);
      const locDataAccu = await locRes.json();
      
      if (!locDataAccu || !locDataAccu.Key) {
         setResult({ willRain: false, message: 'AccuWeather location matching failed.' });
         setChecking(false); return;
      }

      const weatherRes = await fetch(`https://dataservice.accuweather.com/forecasts/v1/hourly/12hour/${locDataAccu.Key}?apikey=${ACCUWEATHER_API_KEY}&details=true`);
      const weatherData = await weatherRes.json();
      
      if (weatherData && weatherData.length >= 3) {
        const probs = weatherData.slice(0, 3).map(h => h.PrecipitationProbability);
        ...
        setResult({ willRain: false, message: 'Could not fetch AccuWeather data.' });
      }
    } catch (err) {
      setResult({ willRain: false, message: 'Failed to fetch AccuWeather data. Please check API quota.' });
    }
    setChecking(false);
  };
  ...
  const handleSuggestionClick = (sug) => {
    ...
    const name = sug.display_name.split(',')[0];
    setSearchQuery(name);
    setShowSuggestions(false);
    checkRainByCoords(lat, lon, name);
  };
  ...
  const handleMapClick = async (lat, lng) => {
    ...
    try {
      const name = await reverseGeocode(lat, lng);
      setSearchQuery(name);
      checkRainByCoords(lat, lng, name);
    } catch (e) {
      setSearchQuery('Selected Location');
      checkRainByCoords(lat, lng, 'Selected Location');
    }
  };
  ...
  const handleFormSubmit = (e) => {
    e.preventDefault();
    if (searchQuery) {
      // If user just types and hits enter, just use the first suggestion or re-geocode
      fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(searchQuery + ', Dhaka, Bangladesh')}&limit=1`)
        .then(res => res.json())
        .then(data => {
           if (data.length > 0) {
              ...
              setPosition({ lat, lng: lon });
              setShowSuggestions(false);
              checkRainByCoords(lat, lon, data[0].display_name.split(',')[0]);
           }
        });
    }
  };
  ...
  return (
    <div className="rain-warning-tool">
      <div className="tool-intro">
        <h3>☔ Rain Commute Check</h3>
        <p>Before you leave home or office, check if it's going to rain on your route using AccuWeather live radar.</p>
      </div>
      
      <div style={{ position: 'relative', height: '250px', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--line)', marginBottom: '20px' }}>
        <MapContainer center={[23.8103, 90.4125]} zoom={12} style={{ height: '100%', width: '100%' }}>
          <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
          <MapClickHandler onMapClick={handleMapClick} enabled={!checking} />
          <Marker position={[position.lat, position.lng]} />
          <MapFly center={[position.lat, position.lng]} />
        </MapContainer>
        <div style={{ position: 'absolute', top: 10, left: '50%', transform: 'translateX(-50%)', background: 'rgba(13,17,23,0.9)', color: '#fff', padding: '6px 12px', borderRadius: '20px', fontSize: '12px', zIndex: 1000, pointerEvents: 'none', border: '1px solid var(--line)' }}>
      ...
      <form onSubmit={handleFormSubmit} className="rp-form" style={{ background: 'var(--surface)', padding: '20px', borderRadius: '12px', border: '1px solid var(--line)', marginBottom: '20px', position: 'relative' }}>
        <div className="form-group" style={{ position: 'relative' }}>
          <label>Search Area / Route:</label>
          <input 
            value={searchQuery} 
            onChange={handleSearchChange} 
            placeholder="e.g. Mirpur 10, Gulshan" 
            autoComplete="off"
          />
          {showSuggestions && suggestions.length > 0 && (
            <ul style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '8px', zIndex: 1001, listStyle: 'none', padding: 0, margin: '4px 0 0', maxHeight: '150px', overflowY: 'auto', boxShadow: '0 4px 12px rgba(0,0,0,0.5)' }}>
              {suggestions.map((s, i) => (
                <li 
                  key={i} 
                  onClick={() => handleSuggestionClick(s)}
                  style={{ padding: '10px 16px', borderBottom: '1px solid var(--line)', cursor: 'pointer', fontSize: '13px', color: '#f8fafc' }}
                >
                  {s.display_name}
                </li>
              ))}
        ...
        <button type="submit" className="button primary" disabled={checking} style={{ width: '100%' }}>
          {checking ? 'Scanning Radar...' : 'Check Next 3 Hours'}
        </button>
      </form>

      {result && (
        <div style={{ padding: '20px', borderRadius: '12px', background: result.willRain ? 'rgba(244, 63, 94, 0.1)' : 'rgba(47, 191, 113, 0.1)', border: `1px solid ${result.willRain ? 'var(--danger)' : 'var(--success)'}` }}>
          <h4 style={{ margin: '0 0 10px 0', color: result.willRain ? 'var(--danger)' : 'var(--success)' }}>
            {result.willRain ? '⚠️ Rain Warning' : '✅ Clear Skies'}
          </h4>
          <p style={{ margin: '0 0 16px 0', color: '#cbd5e1' }}>{result.message}</p>
          <p style={{ fontSize: '11px', color: 'var(--muted)', margin: 0 }}>Source: AccuWeather 12-Hour Forecast API</p>
        </div>
      )}
    </div>
  );
...
const PrayerTraffic = () => {
  ...
  const [depTime, setDepTime] = useState(initTime);
    ...
    <div className="tool-form">
      ...
      {loading ? (
        ...
        <>
          ...
          {warning ? (
            <div style={{ background: 'rgba(251, 146, 60, 0.12)', padding: '16px', borderRadius: '8px', borderLeft: '4px solid #ea580c', color: '#fb923c', marginBottom: '20px' }}>
              <strong>⚠️ Congestion Warning:</strong><br/>{warning}
            </div>
          ) : (
            <div style={{ background: 'rgba(47, 191, 113, 0.12)', padding: '16px', borderRadius: '8px', borderLeft: '4px solid #16a34a', color: '#2fbf71', marginBottom: '20px' }}>
          ...
          <div style={{ background: 'var(--surface)', padding: '16px', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '13px' }}>
              {Object.entries(timings).filter(([k]) => k !== 'Jummah').map(([name, time]) => (
                <div key={name} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--line)', paddingBottom: '4px' }}>
                  <span>{name}</span>
                  <strong>{time}</strong>
                </div>
              ))}
    ...
    <div className="tool-form">
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {alerts.map(a => (
            <div key={a._id} style={{ background: 'rgba(244, 63, 94, 0.12)', border: '1px solid rgba(244, 63, 94, 0.3)', padding: '16px', borderRadius: '8px' }}>
              <div style={{ color: 'var(--danger)', fontWeight: 'bold', fontSize: '18px', marginBottom: '8px' }}>{a.title}</div>
              <p style={{ margin: '0 0 12px', fontSize: '14px' }}>{a.description}</p>
              <div style={{ fontSize: '12px', color: '#f43f5e', background: 'rgba(244, 63, 94, 0.2)', padding: '6px 10px', borderRadius: '4px', display: 'inline-block' }}>
        ...
        <div style={{ textAlign: 'center', padding: '40px 20px', background: 'rgba(47, 191, 113, 0.12)', borderRadius: '12px', color: '#2fbf71' }}>
...
const VipAlerts = () => {
  ...
  return (
    <div className="tool-form">
      ...
      <div style={{ height: '300px', marginBottom: '20px', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--line)' }}>
        <MapContainer center={[23.8103, 90.4125]} zoom={12} style={{ height: '100%', width: '100%' }}>
    ...
    <div className="tool-form">
      ...
      {tab === 'browse' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {items.length === 0 ? <p style={{ textAlign: 'center', color: 'var(--muted)' }}>No active reports.</p> : null}
          {items.map(i => (
            <div key={i.id} style={{ background: i.type === 'lost' ? 'rgba(244, 63, 94, 0.1)' : 'rgba(47, 191, 113, 0.1)', padding: '16px', borderRadius: '8px', border: `1px solid ${i.type==='lost'?'rgba(244,63,94,0.3)':'rgba(47,191,113,0.3)'}` }}>
              <div style={{ fontWeight: 'bold', fontSize: '16px', color: i.type === 'lost' ? 'var(--danger)' : 'var(--success)' }}>
                {i.type.toUpperCase()}: {i.item}
              </div>
              <div style={{ fontSize: '13px', marginTop: '6px', color: '#cbd5e1' }}>
...
const AtmFinder = () => {
  ...
  return (
    <div className="tool-form">
      ...
      <div style={{ height: '350px', marginBottom: '20px', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--line)', position: 'relative' }}>
        ...
        {chapters.map((chap, i) => (
          <div key={i} style={{ border: '1px solid var(--line)', borderRadius: '8px', overflow: 'hidden' }}>
            <button 
              ...
              style={{ width: '100%', padding: '16px', background: activeChap === i ? 'rgba(255,255,255,0.06)' : 'transparent', border: 'none', textAlign: 'left', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between' }}
            >
              {chap.title}
              <span>{activeChap === i ? '▲' : '▼'}</span>
            </button>
            {activeChap === i && (
              <div style={{ padding: '16px', background: 'var(--surface)', borderTop: '1px solid var(--line)', fontSize: '14px', lineHeight: '1.5' }}>
                {chap.content}
              </div>
            )}
          </div>
...
const TravelDiary = () => {
  ...
  return (
    <div className="tool-form">
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
        <div style={{ flex: 1, background: 'rgba(47, 191, 113, 0.12)', padding: '12px', borderRadius: '8px', textAlign: 'center', border: '1px solid rgba(47, 191, 113, 0.3)' }}>
          <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#2fbf71' }}>৳{totalCost}</div>
          <div style={{ fontSize: '12px', color: '#86efac' }}>Total Spend</div>
        </div>
        <div style={{ flex: 1, background: 'rgba(251, 146, 60, 0.12)', padding: '12px', borderRadius: '8px', textAlign: 'center', border: '1px solid rgba(251, 146, 60, 0.3)' }}>
          <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#fb923c' }}>{avgDuration}m</div>
          <div style={{ fontSize: '12px', color: '#fdba74' }}>Avg Commute</div>
        </div>
      </div>

      <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', background: 'var(--surface)', padding: '16px', borderRadius: '8px', marginBottom: '20px', border: '1px solid var(--line)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {logs.map(log => (
          <div key={log.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px', border: '1px solid var(--line)', borderRadius: '6px' }}>
            <div>
              <div style={{ fontWeight: 'bold', fontSize: '14px' }}>{log.from} → {log.to}</div>
              <div style={{ fontSize: '12px', color: 'var(--muted)' }}>{log.date} • {log.mode}</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontWeight: 'bold', color: 'var(--success)' }}>৳{log.cost}</div>
              <div style={{ fontSize: '12px', color: 'var(--muted)' }}>{log.duration} min</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {tab === 'schedule' && (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
            ...
            <tbody>
              {trains.map(t => (
                <tr key={t.no} style={{ borderBottom: '1px solid var(--line)' }}>
                  <td style={{ padding: '10px' }}><strong>{t.name}</strong><br/><span style={{ color: 'var(--muted)', fontSize: '11px' }}>{t.no}</span></td>
          ...
          <p style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '12px', textAlign: 'center' }}>Source: Bangladesh Railway. Subject to change.</p>
        </div>
      )}

      {tab === 'track' && (
        <div style={{ height: '500px', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--line)' }}>
          <iframe 
